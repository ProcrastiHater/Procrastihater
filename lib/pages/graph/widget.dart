...
///*********************************
List<BarChartGroupData> generateWeeklyChart(
...
/// Name: tallestDayBar
///
/// Description: Return the height of the
/// tallest bar in a given week graph
///*********************************
double tallestDayBar(Map<String, Map<String, Map<String, dynamic>>> data) {
  ...
  for (int i = 0; i < data.length; i++) {
    ...
    double dayHeight = 0;
    //Get the height for each day
    for (var appName in dayData.keys) {
      dayHeight += dayData[appName]?['hours'] ?? 0.0;
    }
    //If new height is greater than previous height
    if (tallestHeight < dayHeight) {
      tallestHeight = dayHeight;
    }
  }
  //If the tallest height is greater than .5 or a whole number, then round up to the next whole number
  //Otherwise, round up
  if (tallestHeight - tallestHeight.floor() > .5 ||
      tallestHeight - tallestHeight.floor() == 0) {
...
///*********************************
BarChartGroupData generatedWeeklyGroupData(
    int index, Map<String, Map<String, dynamic>> dailyData) {
...
/// individual bars
///*********************************
List<BarChartGroupData> generateDailyChart(
...
/// individual app data
///*********************************
BarChartGroupData generatedDayData(
    int index, String appName, Map<String, dynamic> appData) {
...
///*********************************
BarTouchData getBarWeekTouch(
    Map<String, Map<String, Map<String, dynamic>>> data,
    void Function(String) onDaySelected) {
  return BarTouchData(
      enabled: true,
